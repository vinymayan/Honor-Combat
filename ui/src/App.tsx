...
function App() {
    ...
    createEffect(() => {
        ...
        const resolveSegment = async (index: number) => {
            let principalIndex = index === 7 && current.mergeDirection8With1 ? 0 : index
            while (principalIndex > 0 && current.mergeWithPrevious[principalIndex]) principalIndex--
            const explicit = current.segmentImages[principalIndex]
            const shared = current.sharedSegmentImage
            ...
            const path = await loadFirstAsset(candidates, generation, getGeneration)
            if (generation === assetGeneration) {
                setSegmentAssets((previous) => previous.map((item, itemIndex) => itemIndex === index ? path : item))
            }
        }
        for (let index = 0; index < 8; index++) void resolveSegment(index)
